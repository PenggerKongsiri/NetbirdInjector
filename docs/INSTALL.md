...
3. Installs NetBird from the official apt repository with a dedicated `signed-by` keyring and enables its service.
4. Runs `netbird up` for browser/device enrollment and verifies management connectivity. For self-hosted NetBird, enter an HTTPS management URL at the prompt or pass `--netbird-management-url https://netbird.example.com`.
5. Runs `npm ci --ignore-scripts`, `npm run check`, `npm audit --audit-level=high`, builds the release, and verifies every release file against `RELEASE_MANIFEST.json`.
6. Runs `setup install`, or the transactional backup/health-gated `setup update` when an existing installation is detected.

The bootstrap never requests or stores a NetBird setup key. If your organization requires setup-key enrollment, run the exact command generated by your NetBird dashboard first, then invoke:
