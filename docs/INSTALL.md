...
2. Keeps Node.js when it is within `24.15–24.x`; otherwise downloads the current official Node.js 24 Linux archive and refuses to install it unless its published SHA-256 matches. The systemd service uses a fixed root-owned search path that supports both the official `/usr/local/bin/node` location and a distribution `/usr/bin/node` installation.
...
5. Runs `npm ci --ignore-scripts`, the applicable project check, `npm audit --audit-level=high`, builds the release, and verifies every release file against `RELEASE_MANIFEST.json`. A Git checkout uses `npm run check`, including the full pushed-history audit. An immutable GitHub source archive uses `npm run check:source-archive` because archives contain no `.git` metadata; it still runs syntax/configuration validation, the current-tree secret scan, and every Node test, while the exact commit's full-history audit remains a required CI gate. Release construction normalizes directories to `0755`, ordinary files to `0644`, and only the five public shell entry points to `0755`, then re-extracts and verifies the archive without allowing the caller's `umask` to change those stored modes.
6. Runs `setup install`, or the transactional backup/health-gated `setup update` when an existing installation is detected.

To update an existing managed VM from the public repository, rerun the same immutable-commit `curl` command from the top of this guide. The bootstrap detects the installed `current` symlink and configuration, leaves an existing NetBird client unchanged, and selects `setup update`. That path creates a checksummed backup, verifies the new release manifest, restarts the service, and automatically reinstates the prior code if the unit or health check fails. It preserves routes, history, administrator settings, 2FA, certificates, and configuration.

### Retrying an interrupted first installation

An initial service failure can leave the manifest-verified release and `config.json` in place before SQLite has created `state.db`. A rerun recognizes the installation and enters the update path. When the standard managed database is absent, setup now stops rather than attempting a read-only backup of a nonexistent file.

For this one incomplete-first-start state, setup explains the recovery and asks:

```text
Recover the interrupted initial installation? [y/N]
```

Approve only when the first installation never became usable and therefore never held routes, account changes, or 2FA state. Approval preserves `config.json`, the administrator password hash, and private-HTTPS certificate files; it creates an empty schema with that initial administrator, immediately creates the normal checksummed pre-update backup, installs the new release, and requires service health. It does not change NetBird.

Recovery fails closed when the service is active, the database entry is a symlink, the configured database is not the standard managed path, or any earlier backup manifest exists. In those cases, investigate and restore a verified backup rather than initializing empty state. Noninteractive staging automation must opt in explicitly with `NIM_RECOVER_MISSING_STATE=1`; never set that variable on an installation that previously held data.
