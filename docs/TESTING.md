...
`npm run check` includes syntax, example-configuration validation, current-tree and pushed-Git-history secret/generated-file/machine-path scanning, and all Node unit/integration tests. CI fetches full Git history for this check. `npm run check:source-archive` is the explicit no-`.git` variant used only by immutable source archives; it retains every applicable check and test but cannot inspect history that the archive does not contain. The lifecycle suite proves that archive and checkout bootstraps select the intended command and that the full history audit still fails closed without Git metadata. The browser smoke launches the local fake environment and pinned Chromium; it covers invalid/valid login, the traffic map, the combined endpoint control, the HTTPS-only verification switch and warning, Optional settings, account settings, profile clone/delete, exact two-tag Umami extraction, attaching a profile to a new site, adding HTML and inline JavaScript to a safe draft, destination testing, Advanced mode, stored-content escaping, fake peer discovery, mobile viewport, keyboard focus, and logout. Administrative API integration tests cover throttling, CSRF, the bounded Umami parse endpoint, stored-content escaping, route/profile transactions, preview, import/export, enable/disable, and exact rollback.

The Docker lifecycle test uses a normal Debian/Node container and a mocked `systemctl`. It validates install/update/rollback/preservation logic and file permissions. It syntax-checks both installer scripts, verifies their help paths, rejects traversal in the remote source-archive allowlist, verifies restrictive Node archive modes are safely normalized, starts the service with Node available only at `/usr/local/bin/node`, builds/re-extracts the release under `umask 077` while enforcing deterministic runtime modes, proves an existing NetBird client causes no NetBird command or service/enrollment action, and proves that unknown bootstrap options and non-HTTPS self-hosted management URLs fail before installation. It also reproduces a first service start that fails before `state.db` exists, proves recovery refuses a symlink, an earlier backup manifest, and missing explicit approval, then validates approved recovery through a normal backup and update. The continued lifecycle binds admin to the container's private IP over native HTTPS, requires certificate-verifying health/doctor success, and verifies a successful private-HTTPS install returns success before backup/restore, failed-update rollback, preserving uninstall, reinstall, and explicit purge. Release unit tests also reject wrong executable, ordinary-file, and directory modes plus symbolic links; a static systemd test prevents regression to a hardcoded `/usr/bin/node`. The lifecycle test does not execute the remote GitHub download or the bootstrap's real apt, Node download, missing-client NetBird repository/enrollment, actual systemd, boot, or reboot paths. See [SANDBOX.md](SANDBOX.md) for container functional and soak commands.
